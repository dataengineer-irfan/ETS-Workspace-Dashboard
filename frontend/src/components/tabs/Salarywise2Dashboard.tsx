...
export const Salarywise2Dashboard: React.FC<Salarywise2DashboardProps> = ({ data, loading, onSelectEmployee }) => {
  ...
  const [activeTrendLens, setActiveTrendLens] = useState<'band' | 'growth' | 'roster'>('band');
  const PAGE_SIZE = 8;
  ...
  return (
    <div className="flex-1 flex flex-col gap-2 overflow-hidden select-none">
      <div className="rounded-xl border p-3" style={{ background: 'linear-gradient(135deg, rgba(16,185,129,0.1), rgba(59,130,246,0.05), rgba(255,255,255,0.18), var(--surface))', borderColor: 'var(--border)', boxShadow: 'var(--shadow-soft)' }}>
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <p className="text-[10px] uppercase tracking-[0.22em] font-semibold" style={{ color: 'var(--muted)' }}>Compensation Trends</p>
            <h2 className="text-xl font-bold tracking-tight mt-1 leading-tight" style={{ color: 'var(--text)' }}>
              Salary progression remains disciplined, with senior promotions and role depth shaping the strongest part of the pay curve.
            </h2>
            <div className="mt-2 flex flex-wrap gap-1.5">
              {[
                { id: 'band', label: 'Band view', active: activeTrendLens === 'band' },
                { id: 'growth', label: 'Growth trend', active: activeTrendLens === 'growth' },
                { id: 'roster', label: 'Roster focus', active: activeTrendLens === 'roster' },
              ].map((lens) => (
                <button
                  key={lens.id}
                  onClick={() => setActiveTrendLens(lens.id as 'band' | 'growth' | 'roster')}
          ...
          <div className="rounded-xl border px-3 py-2 text-right shrink-0" style={{ background: 'var(--panel)', borderColor: 'var(--border)' }}>
            <div className="text-[9px] uppercase tracking-[0.18em]" style={{ color: 'var(--muted)' }}>Matched Records</div>
            <div className="font-bold text-lg mt-0.5" style={{ color: 'var(--text)' }}>{data.filtered_count ?? data.top_earners.length}</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-12 gap-2 flex-1 min-h-0">
        ...
        <div className="col-span-5 glass-panel rounded-xl p-2.5 flex flex-col justify-between overflow-hidden">
          <div className="flex items-center justify-between border-b border-slate-100 pb-1.5 shrink-0">
            <span className="text-xs font-bold text-slate-800 tracking-tight">Salary Trend</span>
      ...
      <div className="grid grid-cols-12 gap-2 flex-1 min-h-0">
        <div className="col-span-8 glass-panel rounded-xl p-2.5 flex flex-col justify-between overflow-hidden">
          <div className="flex items-center justify-between border-b border-slate-100 pb-1.5 shrink-0">
            ...
            <span className="text-[10px] text-amber-700 bg-amber-50 px-1.5 py-0.5 rounded border border-amber-200 font-semibold">Stacked overview</span>
          </div>
          <div className="flex-1 min-h-0 pt-1">
            <ResponsiveContainer width="100%" height="100%">
