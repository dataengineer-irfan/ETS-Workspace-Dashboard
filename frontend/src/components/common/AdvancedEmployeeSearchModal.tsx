  ...
  return (
    <div className="fixed inset-0 z-[100000] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden">
        {/* Header */}
        <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between bg-gradient-to-r from-slate-50 to-cyan-50/40">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-base font-bold text-slate-900">Advanced Employee Search & Talent Finder</h2>
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-cyan-100 text-cyan-950 border border-cyan-300 font-mono">
                  {employeeList.length} Total Records
                </span>
              </div>
              <p className="text-xs text-slate-500">Multi-parameter search across performance, geography, hierarchy and compensation</p>
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="flex items-center gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full bg-white border border-slate-300 rounded-xl pl-9 pr-9 py-2 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:border-cyan-600 focus:ring-2 focus:ring-cyan-100 shadow-xs"
                autoFocus
              />
              {search && (
                <button
            ...
            {hasActiveFilters && (
              <button
                onClick={handleReset}
                className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-rose-600 bg-rose-50 hover:bg-rose-100 border border-rose-200 rounded-xl transition-colors shrink-0"
              >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 text-xs">
            <div>
              ...
              <select
                ...
                className="w-full bg-white border border-slate-300 rounded-lg px-2 py-1 text-xs text-slate-700 focus:outline-none focus:border-cyan-600"
              >
            ...
            <div>
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block mb-1">Job Grade</label>
              <select
                value={selectedGrade}
        ...
        {/* Results Bar */}
        <div className="px-6 py-2 bg-slate-100/70 border-b border-slate-200 flex items-center justify-between text-xs text-slate-600">
          <div className="flex items-center gap-2">
            <span className="font-semibold text-slate-800">
              Showing {filteredEmployees.length} of {employeeList.length} employees
            </span>
            {hasActiveFilters && (
              <span className="text-[10px] bg-cyan-100 text-cyan-800 px-1.5 py-0.5 rounded font-medium">Filtered</span>
            )}
          </div>
          <div className="flex items-center gap-2 text-xs">
            ...
            <button
              ...
              className={`px-2 py-0.5 rounded text-[11px] font-medium flex items-center gap-1 ${sortBy === 'ctc' ? 'bg-cyan-600 text-white' : 'bg-white text-slate-600 border border-slate-200'}`}
            >
              <span>CTC</span>
            ...
            <button
              ...
              className={`px-2 py-0.5 rounded text-[11px] font-medium flex items-center gap-1 ${sortBy === 'grade' ? 'bg-cyan-600 text-white' : 'bg-white text-slate-600 border border-slate-200'}`}
            >
              <span>Grade</span>
            ...
            <button
              ...
              className={`px-2 py-0.5 rounded text-[11px] font-medium flex items-center gap-1 ${sortBy === 'exp' ? 'bg-cyan-600 text-white' : 'bg-white text-slate-600 border border-slate-200'}`}
            >
              <span>Exp</span>
            ...
            <button
              ...
              className={`px-2 py-0.5 rounded text-[11px] font-medium flex items-center gap-1 ${sortBy === 'name' ? 'bg-cyan-600 text-white' : 'bg-white text-slate-600 border border-slate-200'}`}
            >
              <span>Name</span>
        ...
        {/* Results List */}
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 divide-y divide-slate-100">
          {filteredEmployees.length > 0 ? (
                ...
                return (
                  <div
                    ...
                    className={`p-3 rounded-xl border transition-all cursor-pointer flex items-center justify-between gap-3 ${
                      isSelected
                        ? 'bg-cyan-50/70 border-cyan-400 ring-1 ring-cyan-300 shadow-sm'
                    ...
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          <span className="text-[10px] font-bold px-1.5 py-0.2 rounded bg-cyan-100 text-cyan-950 border border-cyan-300 font-mono">
                            {emp['JOB LEVEL']}
                          </span>
                        </div>
                        <p className="text-[11px] text-slate-600 truncate">{emp['JOB TITLE']} · {emp['DEPARTMENT']}</p>
                        <div className="flex items-center gap-2 text-[10px] text-slate-500 mt-0.5">
                          ...
                          {emp['Project Working'] && (
                            <span>· Proj: <strong className="text-slate-700">{emp['Project Working']}</strong></span>
                          )}
                      ...
                      <button
                        className={`text-[10px] font-bold px-2 py-1 rounded-lg flex items-center gap-1 transition-colors ${
                          isSelected
                            ...
                            : 'bg-slate-100 text-cyan-700 hover:bg-cyan-100'
                        }`}
                      >
                        {isSelected ? (
                          <>
            ...
            <div className="text-center py-16 text-slate-400 text-xs">
              <p className="font-semibold text-slate-600">No employees match your search criteria.</p>
        ...
        <div className="px-6 py-3 bg-slate-50 border-t border-slate-200 flex items-center justify-between text-xs text-slate-500">
          ...
          <button
            onClick={onClose}
            className="px-4 py-1.5 rounded-lg border border-slate-300 bg-white text-slate-700 font-semibold hover:bg-slate-100 transition-colors"
          >
            Close
          </button>
        </div>
